...
def loadForrest(num):
	a = []
	for i in range(num):
		a.append(joblib.load("tree"+"%02d"%(i)+".pkl"))
	return a

def testForrest(clf,data):
	a = []
	for tree in clf:
		a.append(tree.predict(data))
	a = numpy.array(a)
	a = a.transpose()
	a = a.astype(int)
	b = []
	for i in range(a.shape[0]):
		counts = numpy.bincount(a[i])
		b.append(numpy.argmax(counts))
	return b

def refine(signal,SIGNAL_SIZE):
...
def SVMTest(svm,data):
	if svm == 0:
		print("svm not yet loaded")
		return []
	return svm.predict(data)

def loadTree():
	return joblib.load('tree.pkl')

def TreeTest(clf,data):
	if clf == 0:
		print("clf tree not yet loaded")
		return []
